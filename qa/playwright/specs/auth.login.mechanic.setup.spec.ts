...
const authFile = resolve(process.cwd(), 'playwright/fixtures/auth.storage.mechanic.json');

/**
 * Setup test: Records mechanic authentication session
 * Run this manually with: pnpm record:mechanic
 */
setup('authenticate as mechanic', async ({ page }) => {
  const email = process.env.QA_MECHANIC_EMAIL;
  const password = process.env.QA_MECHANIC_PASSWORD;

  if (!email || !password) {
    console.log('⏭️  Mechanic credentials not set, skipping authentication setup');
    setup.skip();
    return;
  }

  console.log(`\n🔐 Logging in as mechanic: ${email}`);
  ...
  await page.waitForURL(/\/mechanic/, {
    timeout: 30000,
  }).catch(async () => {
    // If URL doesn't change, check for success indicators
    await page.waitForSelector('[data-authenticated="true"], [data-user-type="mechanic"]', {
      timeout: 10000,
    });
  });
  ...
  console.log('✅ Mechanic authentication successful');
