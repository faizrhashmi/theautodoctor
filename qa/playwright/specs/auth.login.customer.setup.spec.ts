...
const authFile = resolve(process.cwd(), 'playwright/fixtures/auth.storage.customer.json');

/**
 * Setup test: Records customer authentication session
 * Run this manually with: pnpm record:customer
 */
setup('authenticate as customer', async ({ page }) => {
  ...
  if (!email || !password) {
    console.log('⏭️  Customer credentials not set, skipping authentication setup');
    setup.skip();
    return;
  }
  ...
  await page.waitForURL(/\/customer|\/dashboard|\/home/, {
    timeout: 30000,
  }).catch(async () => {
    // If URL doesn't change, check for success indicators
    await page.waitForSelector('[data-authenticated="true"], [data-user-type="customer"]', {
      timeout: 10000,
    });
  });
