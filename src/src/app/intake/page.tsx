...
// Force dynamic rendering (no prerender at build)
export const dynamic = 'force-dynamic';
export const revalidate = 0;

export default function IntakePage() {
  return (
    <Suspense fallback={<div className="p-6 text-sm text-slate-600">Loading intake form…</div>}>
      <IntakeInner />
    </Suspense>
  );
}
...
function IntakeInner() {
  ...
  const canSubmit = (() => {
    ...
    const notUploading = uploads.every(u => u.status !== 'uploading');
