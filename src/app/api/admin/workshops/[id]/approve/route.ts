...
import { workshopApprovalEmail } from '@/lib/email/workshopTemplates'
...
export async function POST(
  ...
  try {
    ...
    if (updateError) {
      console.error('[ADMIN WORKSHOPS] Error approving workshop:', updateError)
    ...
    await supabaseAdmin.from('admin_actions').insert({
      ...
      target_id: id,
      notes: notes || 'Workshop application approved',
      metadata: {
    ...
    await trackApprovalEvent('workshop_approved', {
      workshopId: id,
    ...
    // Send approval email to workshop
    try {
      const dashboardUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/workshop/dashboard`
      const emailTemplate = workshopApprovalEmail({
        workshopName: workshop.name,
        contactName,
        dashboardUrl,
        notes,
      ...
      console.log(`[ADMIN WORKSHOPS] Approval email sent to ${workshop.email}`)

      // Track successful email
      await trackEmailEvent('email_approval_sent', {
        workshopId: id,
        metadata: {
          to: workshop.email,
          workshopName: workshop.name,
      ...
      console.error('[ADMIN WORKSHOPS] Failed to send approval email:', emailError)

      // Track failed email
      await trackEmailEvent('email_approval_failed', {
        workshopId: id,
        success: false,
