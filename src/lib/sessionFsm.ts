...
const STATE_TRANSITIONS: Record<SessionState, SessionState[]> = {
  // SCHEDULED → can start, cancel, or expire
  scheduled: ['waiting', 'live', 'cancelled', 'expired'],

  // WAITING → can go live, cancel, or expire (timeout)
  waiting: ['live', 'cancelled', 'expired'],

  // LIVE → can complete, cancel (emergency), or expire (disconnect)
  live: ['completed', 'cancelled', 'expired'],

  // COMPLETED → can refund or archive
  completed: ['refunded', 'archived'],

  // CANCELLED → can refund or archive
  cancelled: ['refunded', 'archived'],

  // EXPIRED → can refund or archive
  expired: ['refunded', 'archived'],

  // REFUNDED → can only archive
  refunded: ['archived'],
