 ...
 * This prevents illegal state jumps like pending â†’ live without going through accepted.
 *
 * Usage:
...
export class SessionTransitionError extends Error {
  constructor(
    ...
    super(
      message ||
      `Invalid session state transition: ${from} â†’ ${to}. ` +
...
export function getTransitionMessage(from: SessionState, to: SessionState): string {
  ...
  if (validation.valid) {
    return `Transition ${from} â†’ ${to} is valid`
  }
  ...
  return `Cannot transition ${from} â†’ ${to}. Valid options: ${allowed.join(', ')}`
}
