  ...
  useEffect(() => {
    const checkExistingSession = async () => {
      try {
        ...
        if (session) {
          console.log('[MechanicLogin] User already logged in, checking if mechanic...');
          ...
          if (profile && profile.role === 'mechanic') {
            console.log('[MechanicLogin] Mechanic already logged in, redirecting to dashboard');
            router.push(next);
          } else {
            await supabase.auth.signOut();
          }
...
fs.writeFileSync('src/app/mechanic/login/page.tsx', content, 'utf8');
console.log('✅ Updated mechanic login page');
